...
public class MapDistance
{
    ...
    public static String getDistance(String lat1Str, String lng1Str, String lat2Str, String lng2Str)
    {
        ...
        double distance = 2.0D * Math.asin(Math.sqrt(Math.pow(Math.sin(difference / 2.0D), 2.0D) +
                Math.cos(radLat1) *
                        Math.cos(radLat2) * Math.pow(Math.sin(mdifference / 2.0D), 2.0D)));
        distance *= EARTH_RADIUS;
        distance = Math.round(distance * 10000.0D) / 10000L;
    ...
    public static Map<String, String> getAround(String latStr, String lngStr, String raidus)
    {
        ...
        Double mpdLng = Double.valueOf(Double.parseDouble((degree.doubleValue() * Math.cos(latitude.doubleValue() * 0.0174532925199433D) + "").replace("-", "")));
