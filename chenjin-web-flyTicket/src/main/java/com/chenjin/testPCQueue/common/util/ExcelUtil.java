...
public class ExcelUtil
{
    ...
    public Workbook doExportXLS(List dateList, String sheetname, boolean isEntity, boolean isXLS)
            throws IOException
    {
        Workbook wb = null;
        if (isXLS)
    ...
    public Workbook doExportXLS(List dateList, String sheetname, boolean isEntity, boolean isXLS, File file)
    ...
    private void createXLSHeader(Workbook wb, Sheet sheet) {
        for (int i = 0; i < this.heanders.length; i++)
            setStringValue(wb, sheet, 0, (short)i, this.heanders[i]);
    }

    private void createXLS(Workbook wb, Sheet sheet, List<Map<String, Object>> dateList)
    {
        for (int i = 1; i <= dateList.size(); i++) {
            Map object = (Map)dateList.get(i - 1);
            for (int j = 0; j < this.beannames.length; j++) {
                if (StringUtils.isEmpty(this.beannames[j])) {
                    doSetCell(wb, sheet, (short)i, (short)j, "");
                } else {
                    Object value = object.get(this.beannames[j]);
                    Map format = (Map)this.formatMap.get(this.beannames[j]);
                    if ((value != null) && (format != null)) {
                        value = format.get(value);
                    }
                    doSetCell(wb, sheet, (short)i, (short)j, value);
                }
                doCellLine(wb, sheet, (short)i, (short)j);
            }
        }
    }
    ...
    private void doCellLine(Workbook wb, Sheet sheet, int rowNum, int colNum) {
        if ((this.lineMap != null) && (this.lineMap.get(this.beannames[colNum]) != null) && (((Boolean)this.lineMap.get(this.beannames[colNum])).booleanValue())) {
            Cell cell = getMyCell(sheet, rowNum, colNum);
