...
public class ProductDao extends HibernateDaoSupport {
	...
	public List<Product> findByPname(String pname,int begin, int limit) {
		System.out.println(pname);
		String hql = "from Product where pname like '%"+pname+"%'";
		List<Product> list =  this.getHibernateTemplate().execute(new PageHibernateCallback<Product>(hql, null, begin, limit));
		if(list != null && list.size() > 0){
			return list;
		}
	...
	public int findCountPrice(Double low,Double high) {
		String hql = "select count(*) from Product where shop_price between ? and ?";
		List<Long> list = this.getHibernateTemplate().find(hql, new Object[]{low,high});
		if(list != null && list.size() > 0){
			return list.get(0).intValue();
		}
		return 0;
	...
	public List<Product> findByPrice(Double low,Double high,int begin, int limit) {
		String hql = "from Product where shop_price between ? and ?";
		List<Product> list = this.getHibernateTemplate().execute(
				new PageHibernateCallback<Product>(hql, new Object[] { low,high },begin, limit));
		if(list != null && list.size() > 0){
			return list;
		}
		return null;
