...
public class OrderDao extends HibernateDaoSupport {
	...
	public int findCountByPay(Integer uid,Integer statee) {
		//System.out.println(statee);
		String hql = "select count(*) from Order o where o.user.uid = ? and o.state=?";
		//Object[] value={uid,statee};
		List<Long> list = this.getHibernateTemplate().find(hql, new Object[]{uid,statee});
	...
	public List<Order> findPageByPay(Integer uid, Integer statee,int begin, int limit) {
		//System.out.println(statee);
		String hql = "from Order o where o.user.uid = ? and o.state=? order by o.ordertime desc";
		List<Order> list = this.getHibernateTemplate().execute(
				new PageHibernateCallback<Order>(hql, new Object[] { uid,statee },begin, limit));
		if (list != null && list.size() > 0) {
			return list;
		}
		return null;
